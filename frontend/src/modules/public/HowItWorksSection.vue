<template>
  ...
  >
    ...
    <!-- Content -->
    <div
      class="relative z-10 max-w-7xl w-full px-4 sm:px-6 lg:px-20 text-center"
    >
      <h2
      ...
      <p
        class="mt-4 sm:mt-6 animate-fadeUp delay-150 drop-shadow-md mx-auto"
        style="
      ...
      >
        <div
          ...
          :key="step.number"
          class="step-card rounded-2xl p-5 sm:p-6 shadow-lg flex flex-col items-center text-center animate-fadeUp delay-150"
          :class="[
            colorClasses[idx % colorClasses.length].bg,
            colorClasses[idx % colorClasses.length].text,
          ]"
        >
          <!-- Step Number -->
          <div
            class="font-extrabold mb-3"
          ...
          <!-- Step Description -->
          <p :style="{ fontSize: 'clamp(0.875rem, 1.8vw, 1rem)' }">
            {{ step.description }}
          </p>
        </div>
      </div>
