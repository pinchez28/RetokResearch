<template>
  <div class="p-4 sm:p-6 md:p-8 max-w-7xl mx-auto space-y-6">
    <h1 class="text-2xl sm:text-3xl font-bold mb-4">Completed Jobs</h1>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
      <input
        ...
        class="w-full sm:w-1/2 p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#0046FF]"
      />
      <select
        v-model="filterStatus"
        class="w-full sm:w-1/4 p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#0046FF]"
      >
      ...
      <table class="min-w-full divide-y divide-gray-200">
        <thead class="bg-[#001BB7] text-white">
          <tr>
            ...
            <th class="px-4 py-3 text-left cursor-pointer" @click="sortBy('client')">
              Client <span>{{ getSortIcon("client") }}</span>
            </th>
            <th class="px-4 py-3 text-left cursor-pointer" @click="sortBy('expert')">
              Expert <span>{{ getSortIcon("expert") }}</span>
            </th>
            <th class="px-4 py-3 text-left cursor-pointer" @click="sortBy('date')">
              Completed Date <span>{{ getSortIcon("date") }}</span>
            </th>
    ...
    <div class="lg:hidden grid gap-4">
      ...
      >
        <div class="flex justify-between items-start mb-2">
          <h2 class="font-semibold text-lg text-gray-800">{{ job.id }}</h2>
          <span
            :class="{
              'bg-green-200 text-green-800 px-2 py-1 rounded-full text-sm':
                job.status === 'completed',
        ...
        <p class="text-gray-600">
          <span class="font-medium">Expert:</span> {{ job.expert }}
        </p>
        <p class="text-gray-600">
          <span class="font-medium">Completed:</span> {{ job.date }}
        </p>
    ...
    <!-- Pagination -->
    <div
      class="mt-4 flex flex-col sm:flex-row justify-between items-center gap-2 sm:gap-0"
    >
      <button
        @click="prevPage"
        :disabled="currentPage === 1"
        class="px-4 py-2 bg-gray-200 rounded-md disabled:opacity-50 hover:bg-gray-300 transition"
      >
        Previous
      </button>
