    ...
    >
      ...
      >
        ...
        >
          ...
          <li v-if="!selectedClient.projects?.length" class="text-gray-500">
            No projects yet.
          </li>
        </ul>
...
// -------------------- OPEN MODAL AND FETCH PROJECTS --------------------
const openModal = async (client) => {
  try {
    const { data } = await api.get(`/admin/clients/${client._id}/projects`);
    selectedClient.value = { ...client, projects: data };
  } catch (err) {
    console.error('Failed to fetch projects', err);
    selectedClient.value = { ...client, projects: [] };
  }
};
...
const updateStatus = async (status) => {
  ...
  try {
    const { data } = await api.put(
      `/admin/clients/${selectedClient.value._id}`,
      { status }
    );
    ...
    Swal.fire('Success', `Client status updated to ${status}`, 'success');
  } catch (err) {
