<template>
  <div class="p-6 md:p-10 space-y-8">
    ...
    <div v-else-if="!job" class="text-gray-500 text-center py-10">
      Job not found.
    </div>
    ...
    <section v-if="job" class="bg-white rounded-2xl shadow-lg p-6">
      ...
      >
        <div>
          ...
          <!-- Hired Expert Info -->
          <div v-if="job.hiredExpertId && hiredExpert" class="mt-4">
            <h3 class="text-xl font-semibold text-green-700">Hired Expert</h3>
            <div class="flex items-center gap-4 mt-2">
              <img
                v-if="hiredExpert.photo"
                :src="backendURL + hiredExpert.photo"
                class="w-16 h-16 rounded-full border-2 border-green-500"
              />
              <div>
                <p class="font-semibold">{{ hiredExpert.name }}</p>
                <p class="text-sm text-gray-500">
                  {{ hiredExpert.specialization || '—' }}
                </p>
              </div>
            </div>
          </div>
      ...
      >
        ...
        >
          ...
          <div class="mb-4 space-y-2">
            ...
            <p class="text-gray-700">
              ...
              <span v-if="app.expertSnapshot?.certifications?.length">
                {{ parseCerts(app.expertSnapshot.certifications).join(', ') }}
              </span>
              <span v-else>—</span>
            </p>
          </div>
          ...
          <div class="flex flex-col md:flex-row gap-3">
            ...
            <button
              v-if="app.cvUrl"
              @click="viewCv(app.cvUrl)"
              class="w-full md:w-auto px-6 py-3 rounded-xl border border-blue-500 text-blue-600 font-semibold hover:bg-blue-50"
            >
              View CV
            </button>
    ...
    <div
      v-else-if="job && !applications.length"
    ...
    <div
      v-if="showCvViewer"
      class="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex flex-col"
    >
      <div class="bg-white shadow-xl p-4 flex justify-between items-center">
        <h2 class="text-xl font-bold text-[#001BB7]">Expert CV</h2>
        <button
          @click="closeCvViewer"
      ...
      <div class="flex-1 bg-gray-100">
        <iframe :src="selectedCvUrl" class="w-full h-full border-0"></iframe>
      </div>
    </div>
...
const hiredExpert = ref(null);

const job = computed(() => jobsStore.value.find((j) => j._id === jobId));
const applications = computed(() => job.value?.applications || []);

function populateHiredExpert() {
  if (job.value?.hiredExpertId) {
    const expertApp = job.value.applications.find(
      (a) => a.expertSnapshot?._id === job.value.hiredExpertId
    );
    if (expertApp) hiredExpert.value = expertApp.expertSnapshot;
  } else {
...
watch(() => jobsStore.value, populateHiredExpert, { deep: true });
...
const viewCv = (url) => {
  selectedCvUrl.value = (backendURL + '/' + url).replace(/\\/g, '/');
...
const parseCerts = (certs) => {
  try {
    return certs
      .flatMap((c) =>
        typeof c === 'string' ? JSON.parse(c.replace(/\\?/g, '')) : c
      )
      .filter(Boolean);
  } catch {
    return [];
...
const hireExpert = async (app) => {
  try {
    ...
    const result = await Swal.fire({
      title: 'Hire Expert?',
      text: `Are you sure you want to hire ${app.expertSnapshot.name}?`,
      icon: 'question',
    ...
    if (result.isConfirmed) {
      await api.post(`/client/jobs/${jobId}/hire`, {
        expertId,
      ...
      Swal.fire(
        'Hired!',
        `${app.expertSnapshot.name} has been hired.`,
        'success'
      );
    }
  } catch (error) {
    console.error('Hire failed:', error);
    Swal.fire('Error', 'Failed to hire expert.', 'error');
  }
};
