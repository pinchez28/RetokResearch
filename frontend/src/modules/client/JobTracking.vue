<template>
  <div class="p-6 md:p-10 space-y-8">
    ...
    >
      ...
      >
        ...
        <div class="mt-4 flex flex-wrap gap-2">
          <!-- View Job: modal if not assigned, page if assigned -->
          <button
            @click="viewJob(job)"
    ...
    <!-- Empty State -->
    <div v-else class="text-center text-gray-500 py-20">No jobs available.</div>

    <!-- Job Details Modal (only for unassigned jobs) -->
    <div
      v-if="showJobModal"
...
const loadJobs = async () => {
  try {
    const { data } = await axios.get('/client/jobs'); // your axios instance handles baseURL & token
...
const viewJob = (job) => {
  if (job.status === 'in_progress' || job.assignedExpert?._id) {
    router.push({ name: 'JobDetails', params: { jobId: job._id } });
  } else {
...
const goToApplications = (job) => {
  router.push({ name: 'ClientJobApplications', params: { jobId: job._id } });
};
