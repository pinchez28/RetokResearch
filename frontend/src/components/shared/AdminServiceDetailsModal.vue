...
const saveService = async () => {
  loading.value = true;
  try {
    // Convert includes string to array if typed as comma-separated
    ...
    if (props.service._id) {
      ...
      response = await api.put(`/admin/services/${props.service._id}`, form);
    } else {
      // Create service → admin route
      response = await api.post('/admin/services', form);
    }

    emit('saved', response.data.service || response.data);
...
const deleteService = async () => {
  ...
  try {
    await api.delete(`/admin/services/${props.service._id}`);
...
<template>
  <transition name="fade">
    ...
    >
      <div
        class="bg-cream rounded-3xl shadow-2xl w-full max-w-4xl p-8 relative overflow-y-auto max-h-[90vh]"
      >
        <!-- Close Button -->
        <button
          @click="$emit('close')"
